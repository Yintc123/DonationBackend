...
export default async function setup(project: TestProject): Promise<() => Promise<void>> {
  ...
  const { postgres, redis, localstack } = containers

  project.provide('TEST_DATABASE_URL', postgres.connectionUri)
  project.provide('TEST_REDIS_HOST', redis.host)
  project.provide('TEST_REDIS_PORT', String(redis.port))
  ...
  project.provide('TEST_S3_ENDPOINT', localstack.endpoint)
  project.provide('TEST_S3_BUCKET', localstack.bucket)
