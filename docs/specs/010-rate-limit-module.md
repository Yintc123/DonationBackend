...
### 9.1 不套用 rate-limit 的路徑

- `GET /health` / `GET /health/db` / `GET /health/cache`(健康檢查)
...
某些公共 / 匿名端點(若日後出現)可路徑層設定 `failureMode: 'open'`,代表 Redis 不可用時放行;本期**不**有此類端點。
