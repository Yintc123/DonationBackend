...
可用 Fastify v5 的 `disableRequestLogging` + 自寫 hook,或 plugin 內判斷 path 略過。預設用**前者**(*v0.5 — 同步實作*):`createLogger` 在 Fastify constructor 設 `disableRequestLogging: true` 關掉內建雙行 log,改由 `loggerPolicyPlugin`(`src/lib/logger/policy.ts:28-46`,非 `src/plugins/logger.ts`)在 `onRequest` / `onResponse` 重放非排除路徑的 log。排除判斷在 `shouldSkipRequestLog()`(`policy.ts:14-26`):`req.method === 'OPTIONS'`、或 `req.routeOptions?.url`(已解析路由)為 `/health` 或 `startsWith('/health/')`、或 raw `req.url` 命中同條件(涵蓋未解析路由的 404)時 return `true`,不寫 log。
...
*(v0.5 — 同步實作:對齊 `src/lib/logger/redact.ts:14-54`。`*.<KEY>` 為 pino 的巢狀屬性比對,**不是** glob;因此每個 env 變體都要各自列出——`*.JWT_SECRET` 這個 env 名並不存在,真實名為 `JWT_ACCESS_SECRET` / `JWT_REFRESH_SECRET`,須分別列出)*
...
export const REDACT_PATHS = [
  ...
  'req.body.code',              // OAuth authorization code
  // ── config snapshot, infra secrets(見 §11)───────────────────────────
  '*.DB_PASSWORD',
  '*.DATABASE_URL',             // postgresql://user:password@host/db — 內嵌密碼
  '*.REDIS_PASSWORD',
  '*.AWS_SECRET_ACCESS_KEY',    // AWS_ACCESS_KEY_ID 視為使用者名,刻意不 redact
  '*.JWT_ACCESS_SECRET',
  '*.JWT_REFRESH_SECRET',
  '*.GOOGLE_CLIENT_SECRET',     // GOOGLE_CLIENT_ID 為公開識別碼,刻意不 redact
  // ── generic catch-all ──────────────────────────────────────────────
  '*.password',
]
```
...
|---|---|---|---|
| `startup_begin` | server 啟動開始(`src/server.ts`) | info | — |
| `startup_complete` | 所有 plugin register 完成、listen 成功(`src/server.ts`) | info | — |
| `shutdown_begin` | 收到 SIGTERM / SIGINT(`src/server.ts`) | info | — |
| `shutdown_complete` | onClose hook 全部跑完(`src/server.ts`) | info | — |
| `plugin_registered` | 每個 Fastify plugin register 成功 | info | — | *(v0.5 — **尚未實作 / 規劃中**:目前 code 不 emit 此 event;保留供未來逐 plugin 追蹤)* |
| `request_received` / `request_completed` | 由 `loggerPolicyPlugin` 重放(§6.2) | info | — | *(v0.5 — 實作以 `msg` 為 `incoming request` / `request completed`,**未帶 `event` 欄位**;規劃補上)* |

##### 9.3.1a Process-level fatal(spec 005 §9 擁有,由 logger emit)

*(v0.5 — 同步實作:`src/lib/errors/process-handlers.ts:55,73` 以 `logger.fatal` emit 下列 event;登錄於此供聚合查詢)*

| event | 觸發點 | level | audit |
|---|---|---|---|
| `unhandled_rejection` | `process.on('unhandledRejection')`,log fatal 後 graceful drain | fatal | — |
| `uncaught_exception` | `process.on('uncaughtException')`,log fatal 後 `exit(1)` | fatal | — |
...
| 0.5 | 2026-06-23 | **同步實作**:§6.2 請求日誌排除實作位置由 `src/plugins/logger.ts` 更正為 `src/lib/logger/policy.ts:28-46`(`shouldSkipRequestLog`),機制改為 `disableRequestLogging: true` + `loggerPolicyPlugin` 重放,排除判斷用 `routeOptions?.url` / raw url + `OPTIONS`;§7.1 `REDACT_PATHS` 對齊 `redact.ts`——移除不存在的 env `*.JWT_SECRET`,改列真實的 `*.JWT_ACCESS_SECRET` / `*.JWT_REFRESH_SECRET`,補上 `currentPassword` / `newPassword` / `idToken` / `code` / `*.DATABASE_URL` / `*.REDIS_PASSWORD` / `*.AWS_SECRET_ACCESS_KEY`,並註明 `AWS_ACCESS_KEY_ID` / `GOOGLE_CLIENT_ID` 刻意不 redact;§9.3.1 標註 `plugin_registered` / `request_received`(帶 `event` 欄位)**尚未實作**,新增 §9.3.1a 收錄 `unhandled_rejection` / `uncaught_exception`(spec 005 process-handlers emit) |
