...
> **v0.8 — checksum 設定(同步實作)**:`src/lib/s3/client.ts:29-30` 建 `S3Client` 時帶 `requestChecksumCalculation: 'WHEN_REQUIRED'` + `responseChecksumValidation: 'WHEN_REQUIRED'`。原因:AWS SDK v3.729+ 預設 `WHEN_SUPPORTED` 會替 PUT 加 `x-amz-checksum-*` header,LocalStack 3 會以 `InvalidRequest` 拒收 CRC32 變體;且該 header 進入 signed-PUT 契約後,browser 上傳不回傳 checksum → 簽章不符。固定回 `WHEN_REQUIRED` 讓 LocalStack 與真 S3 行為一致(等同 pre-v3.729)。
...
| `{ext}` | 副檔名,小寫。**寫入端(新 key)白名單** `png` / `jpg` / `webp` / `gif`(v0.8 — 同步實作:`ALLOWED_EXTS`,`src/lib/s3/key.ts:27` **不含 `jpeg`**,`image/jpeg` 一律產 `jpg`,見 §5.1.1);**讀取端** regex(`OBJECT_KEY_REGEX`)仍額外容許 `jpeg` 向前相容舊 key | `png` |
...
### 8.1 `objectUrl(key)`（v0.8 — 同步實作:三個 regime)

實際 `src/lib/s3/url.ts` 依優先序有**三個** regime(原 spec 只列兩個,漏了 path-style 分支):

```ts
// src/lib/s3/url.ts
function resolveBase(config: ObjectUrlConfig): string {
  // 1. publicUrlBase 有設 → 直接用(CDN / CloudFront)
  if (config.publicUrlBase) {
    return config.publicUrlBase.replace(/\/+$/, '')
  }
  // 2. forcePathStyle=true → path-style(涵蓋含 dot 的 bucket / MinIO / LocalStack)
  if (config.forcePathStyle) {
    return `https://s3.${config.region}.amazonaws.com/${config.bucket}`
  }
  // 3. 預設 → virtual-hosted style
...
export function objectUrl(key: string, config: ObjectUrlConfig): string {
  return `${resolveBase(config)}/${key}`
}
```

| # | 條件 | 產出 base | 用途 |
|---|---|---|---|
| 1 | `publicUrlBase` 非空 | `<base>`(去尾斜線) | CDN / CloudFront / LocalStack(§9.2 直接填 `http://localhost:4566/<bucket>`) |
| 2 | `forcePathStyle = true` | `https://s3.<region>.amazonaws.com/<bucket>` | path-style;涵蓋含 dot 的 bucket、MinIO 等 virtual-hosted 不支援的情境 |
| 3 | 預設 | `https://<bucket>.s3.<region>.amazonaws.com` | virtual-hosted style(§1.3 v0.1 直接公開 bucket 的形狀) |

> regime 2 的 dot-bucket 邊界在 `config.ts` 載入期就有把關(§4.2:virtual-hosted style 下 bucket 含 dot 直接 throw ConfigError),兩者互補。
...
// v0.8 — 同步實作:實際 AppError class 為 InternalError / ServiceUnavailableError
// (非早期 spec 構想的 ConfigError / TransientError);error code 字串不變。
export function mapS3Error(err: unknown): AppError {
  if (err instanceof S3ServiceException) {
    switch (err.name) {
      case 'NoSuchBucket':    return new InternalError({ code: ErrorCode.S3_BUCKET_MISCONFIGURED, cause: err, ... })
      case 'AccessDenied':    return new InternalError({ code: ErrorCode.S3_ACCESS_DENIED, cause: err, ... })
      case 'TimeoutError':    return new ServiceUnavailableError({ code: ErrorCode.S3_TIMEOUT, cause: err, ... })
      case 'NetworkingError': return new ServiceUnavailableError({ code: ErrorCode.S3_UNREACHABLE, cause: err, ... })
      default:                return new InternalError({ code: ErrorCode.S3_UNKNOWN, cause: err, ... })
    }
  }
  return new InternalError({ code: ErrorCode.S3_UNKNOWN, cause: err, ... })
}
```

> **v0.8 — class 命名校準(同步實作)**:`src/lib/s3/errors.ts:15-38` 用 `InternalError`(config / 未知類:`NoSuchBucket` / `AccessDenied` / default)與 `ServiceUnavailableError`(暫時性:`TimeoutError` / `NetworkingError`),**非**本 spec 早期構想的 `ConfigError` / `TransientError`。**error code 字串完全相同**(`S3_BUCKET_MISCONFIGURED` / `S3_ACCESS_DENIED` / `S3_TIMEOUT` / `S3_UNREACHABLE` / `S3_UNKNOWN`,取自 `ErrorCode` enum),對外行為不變;此處僅 AppError class 名稱對齊。上方 code 為示意(pseudocode),完整建構參數見實作。
...
| 0.8 | 2026-07-07 | 與實作同步(不改 code,只補文件落差):(1) §8.1 `objectUrl()` 補第三個 regime —— 實際 `src/lib/s3/url.ts:26-34` 除 `publicUrlBase` 與 virtual-hosted 外,另有 `forcePathStyle` 分支回 `https://s3.<region>.amazonaws.com/<bucket>`(涵蓋含 dot bucket / MinIO / LocalStack);(2) §5.1 ext 白名單:寫入端 `ALLOWED_EXTS`(`key.ts:27`)**不含 `jpeg`**,讀取端 regex 仍容許 `jpeg` 向前相容;(3) §11.1 SDK error → AppError class 校準:實作用 `InternalError` / `ServiceUnavailableError`(非早期構想的 `ConfigError` / `TransientError`),error code 字串不變,code block 標為 pseudocode;(4) §3.1 補 `client.ts:29-30` checksum 設定 `WHEN_REQUIRED`(LocalStack 相容)|
