...
- **API 版本化策略**(URI vs header)— 由 spec 023 擁有(v0.4 — 同步實作:實際 user-facing 業務 API 掛在 `/user/v{N}` 前綴、`USER_API_VERSIONS = ['v1']`;`/auth/*` 與 `/cms/*` **不**版本化。見 `src/lib/http/api-version.ts:14`、`src/app.ts:146-187`)。**本 spec 各範例沿用 `/v1/<resource>` 作為通用佔位**,實際路徑前綴依 spec 023
- **GraphQL / gRPC** — 不在採用範圍
...
Lookup cache:  jkod:cache:idempotency:{endpointId}:{key}
  hit  → 回 cached response(同 status / body / content-type / location,加 X-Idempotency-Replay: true)
  miss → 執行業務邏輯
         成功(2xx)→ cache response(TTL 24h),回 client
         失敗(4xx / 5xx)→ **不**快取,讓 client 改 input 後可重試
```

> **實作備註(v0.4 — 同步實作)**:replay 只還原 `status` / `content-type` / `location` 三者 + body,**不**逐一還原原始回應的其餘 headers(如 `X-RateLimit-*` / `ETag`);另**額外**加上本節未列出的 `Cache-Control: no-store`(避免中介快取 replay 結果)。見 `src/lib/http/idempotency-plugin.ts:168-176`。

### 7.4 規則

- `Idempotency-Key` 必為 UUID 或 ULID;否則 400 `IDEMPOTENCY_KEY_INVALID`
- 同一 key 於**同一 endpoint** 配不同 body → 第二次回 422 `IDEMPOTENCY_KEY_CONFLICT`(攻擊或 client bug)(v0.4 — 同步實作:儲存 key 以 endpoint 命名空間隔離 `cache:idempotency:{endpointId}:{key}`,`endpointId = sha256(method:path).slice(0,16)`(`src/lib/http/idempotency.ts:67-93`)。**因此「同 key 不同 endpoint」各自產生獨立快取項、永遠不會 lookup 命中對方,無法觸發 CONFLICT**;唯一觸發路徑是同 endpoint、同 key,但 `requestHash`(method+path+body)不符,見 `idempotency-plugin.ts:147-163`)
...
- **API 版本化**:(v0.4 — 已由 spec 023 落地為 `/user/v{N}` URI 版本化,`USER_API_VERSIONS = ['v1']`,`/auth/*` `/cms/*` 不版本化)若日後出現 breaking change,要走「新版 path」(`/user/v2`)還是 header(`Accept: application/vnd.jkod.v2+json`)?待出現第一個 breaking change 時決定
...
| 0.4 | 2026-07-07 | **同步實作**:§7.4 修正「同 key 不同 endpoint → 422 CONFLICT」為**不可能觸發**——儲存 key 以 `endpointId` 命名空間隔離,不同 endpoint 各自獨立快取,唯一 CONFLICT 路徑是同 endpoint 同 key 但 body 不符(`src/lib/http/idempotency.ts:67-93`、`idempotency-plugin.ts:147-163`);§7.3 replay 只還原 status/content-type/location + body,並額外加未文件化的 `Cache-Control: no-store`(`idempotency-plugin.ts:168-176`);§1.3 / §14 版本化改為引用 spec 023 `/user/v{N}`,範例 `/v1/<resource>` 註明為通用佔位(`src/lib/http/api-version.ts:14`) |
| 0.3 | 2026-06-16 | **§7 Idempotency-Key 落地** — 全域 Fastify plugin(`src/lib/http/idempotency-plugin.ts`)實作 §7.3 lookup / replay / cache 流程;UUID v4 / ULID 雙格式接受(§7.4);Redis key `cache:idempotency:{endpointId(sha256 16 hex)}:{key}`;TTL 24h(§7.4);SETNX 解決並發 first-write 競爭;`X-Idempotency-Replay: true` 標示 replay;route-level `config.idempotency = 'required'` 強制要求 header(spec §7.1 金流類);Redis 不可用 → fail-open(交易仍走完,不阻擋正常請求,呼應 spec 010 §11 唯一獨佔關鍵 path)。**§9 Content negotiation 落地** — 同一 plugin 群組加 onRequest hook:`Accept` 不接受 application/json → 415、body-bearing POST/PUT/PATCH 缺 `Content-Type: application/json` → 415;`/health/*` 與 OPTIONS preflight 排除。Error code 字典(spec 005 §4.2.4)`IDEMPOTENCY_KEY_INVALID` / `IDEMPOTENCY_KEY_CONFLICT` 同步註冊 |
